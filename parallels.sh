...
while getopts "t:g:n:p:s:a:" OPTION
do
   case $OPTION in
      t) TAGS="--tags $OPTARG"
         OTAG="-o"   
...
function create_android_emulator(){
      ...
      $ANDROID_HOME/tools/bin/avdmanager -s --silent create avd --force --name ${names[$i]} --abi $aabi --package $api --device $skin &>/dev/null
}
...
# start functional tests
 function start_cucumber_parallel(){
       if [ "$OTAG" == "-o" ]; then
            parallel_cucumber features/ $GROUP_BY $NODES $OTAG "$TAGS"
      else
            parallel_cucumber features/ $GROUP_BY $NODES
      fi
 }
...
function generate_report(){
      report_builder -s 'report/' -o together.html
}
